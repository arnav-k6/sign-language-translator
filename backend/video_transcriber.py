...
def load_model():
    ...
    model_path = os.path.join(_models_dir, MODEL_FILE)
    scaler_path = os.path.join(_models_dir, SCALER_FILE)
...
def transcribe_video_file(video_path: str, mode: str = 'both', confidence_threshold: float = 0.75, min_stable_duration: float = 0.25, return_segments: bool = False):
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    landmarker_path = os.path.join(script_dir, '..', 'models', HAND_LANDMARKER_MODEL)
...
def letter_sequence_to_words(letter_seq: str, dictionary: dict = None) -> list:
    ...
    if dictionary is None:
        dict_path = os.path.join(os.path.dirname(__file__), '..', 'data', 'words_dictionary.json')
